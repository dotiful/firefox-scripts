...
(function() {
    ...
    try {
      ...
      btn.addEventListener(
        ...
        function(e) {
          ...
          if (btn && btn.getAttribute('displaystop')) BrowserStop();
          else BrowserReload();
        },
        false
      );
  ...
  function reloadBtnAttr(e) {
    ...
    if (btn && (!e || e.attrName == 'displaystop')) {
      var newVal = e
        ? e.newValue
        : document.getElementById('reload-button').getAttribute('displaystop');
      if (newVal)
        btn.style.listStyleImage = "url('chrome://browser/skin/stop.svg')";
      else btn.style.listStyleImage = "url('chrome://browser/skin/reload.svg')";
    }
  }
